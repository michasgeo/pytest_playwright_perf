...
async def test_powerbi_load(user_id):
    async with async_playwright() as p:
        ...
        try:
            ...
            # Hover over the visual container first to reveal the drill buttons
            # await page.locator("div.visual-container").first.hover()

            # # Wait for the drill-down button to appear
            # await page.wait_for_selector('[data-testid="drill-down-level-grouped-btn"]', timeout=10000)

            # # Now click the drill-down (fork) button
            #await page.get_by_test_id("drill-down-level-grouped-btn").click()
